...
class NetworkAutomationApp(App):
    ...
    THEMES = {
        "tokyo-night": {
            "name": "Tokyo Night",
            "emoji": "🌃",
            "css": "tokyo_night.tcss"
        },
        "nord": {
            "name": "Nord",
            "emoji": "🏔️",
            "css": "themes/nord.tcss"
        },
        "gruvbox": {
            "name": "Gruvbox",
            "emoji": "🟤",
            "css": "themes/gruvbox.tcss"
        }
    }
    ...
    def action_dashboard(self) -> None:
        """Show dashboard"""
        self.notify("Dashboard is the home screen! Press ESC to return.", severity="information")
 
    def action_toggle_theme(self) -> None:
        """Cycle through available themes"""
        theme_keys = list(self.THEMES.keys())
        current_index = theme_keys.index(self.active_theme)
        next_index = (current_index + 1) % len(theme_keys)
        next_theme = theme_keys[next_index]
        self.switch_theme(next_theme)
 
    def switch_theme(self, theme_key: str) -> None:
        """Switch to a specific theme using Textual's built-in theme system"""
        if theme_key not in self.THEMES:
            self.notify(f"Theme '{theme_key}' not found", severity="error")
            return
        ...
        try:
            ...
            # Update title
            self.title = f"{theme['emoji']} Network Automation TUI - {theme['name']}"
            self.notify(f"Switched to {theme['name']} theme", severity="success")
        except Exception as e:
            self.notify(f"Error switching theme: {str(e)}", severity="error")
    ...
    def show_device_management(self) -> None:
        ...
        from tui.components.simple_inventory_manager import SimpleInventoryManager
        self.push_screen(SimpleInventoryManager(self.inventory_service, self.api_service))
 
    def show_template_editor(self) -> None:
        """Navigate to template editor screen"""
        from tui.components.smart_template_manager import SmartTemplateManager
        self.push_screen(SmartTemplateManager())
...
class HelpScreen(Screen):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container():
            ...
            yield Static("  d - Device Management (Inventory)")
            ...
            yield Static("  h - Help")
            yield Static("  Ctrl+T - Toggle Theme (Tokyo Night / Nord / Gruvbox)")
