...
def get_host_ips() -> List[str]:
    ...
    upgrade_hosts_file = os.path.join(
        os.getenv("VECTOR_PY_DIR", project_dir),
...
def probe_device(
    ...
    while time.time() - start_time < max_wait:
        try:
            ...
            print(
                f"⚠️ {hostname} pingable but PyEZ not ready. Retrying in {interval} seconds..."
            )
        except Exception as e:
    ...
    for attempt in range(max_attempts):
        ...
        try:
            ...
            if not dev.connected:
                logger.error(
                    f"Attempt {attempt + 1}: Connection to {hostname} reported as not connected"
                )
                raise ConnectError(
